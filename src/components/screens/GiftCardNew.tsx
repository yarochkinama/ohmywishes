    ...
    <div className="absolute inset-0 flex flex-col bg-white">
      ...
      <div className="scroll-y flex-1" style={{ padding: '20px 20px 0' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 20, fontWeight: 700, color: '#6BA83A' }}>{formatPrice(gift.price)}</span>
          <ShopBadge shop={gift.shop} />
          <span style={{
        ...
        {/* Owner comment */}
        {gift.ownerComment && (
          <div style={{ marginBottom: 16, padding: '12px 14px', background: '#F9FFF5', borderRadius: 12, borderLeft: '3px solid #6BA83A' }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: '#6BA83A', marginBottom: 6 }}>Комментарий {friend.name}</div>
          ...
          {isFree ? (
            <>
              ...
              <button
                onClick={onBook}
                className="press"
                style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', borderRadius: 14, background: '#6BA83A', border: 'none', cursor: 'pointer', textAlign: 'left' }}
              >
