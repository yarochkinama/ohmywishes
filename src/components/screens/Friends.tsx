          ...
          {tabs.map(t => (
            <button
              ...
              style={{
                ...
                cursor: 'pointer',
                fontSize: 14,
                background: tab === t.id ? '#6BA83A' : 'transparent',
            ...
            {friendRequests.map(r => (
              <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', borderRadius: 14, border: '1px solid #F0F0F0', background: 'white', marginBottom: 8 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  >
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#6BA83A" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>
                  </button>
                  <button
            ...
            {displayFriends.map(f => (
              ...
              >
                <div style={{ position: 'relative' }}>
                  ...
                  {f.isOnline && (
                    <div style={{ position: 'absolute', bottom: 2, right: 2, width: 10, height: 10, borderRadius: '50%', background: '#6BA83A', border: '2px solid white' }} />
                  )}
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 15, fontWeight: 600, color: '#111' }}>{f.name}</div>
                  <div style={{ fontSize: 12, color: f.isOnline ? '#6BA83A' : '#888', marginTop: 2 }}>
